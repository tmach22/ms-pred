...
def train_epoch(epoch: int, scalpel: ICEBERGScalpel, reranker: nn.Module, dataloader: DataLoader, optimizer: torch.optim.Optimizer, cpu_pool: mp.Pool, device: torch.device, checkpoint_dir: str = None, save_every_steps: int = 200):
    ...
    for step, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            valid_frags = scalpel.extract_batch(smiles_list=valid_smiles, collision_engs=valid_ces, precursor_mzs=valid_mzs) if valid_smiles else []
        ...
        rewards = cpu_pool.starmap(magma_score_fragments, scoring_args)
...
def evaluate_epoch(
    ...
    with torch.no_grad():
        for step, batch in enumerate(dataloader):
            ...
            candidate_smiles = [
                [fr.smiles for fr in frags if fr.smiles]
            ...
            feat_matrix = build_reranker_features(
                frag_lists, batched_ces.cpu(), batched_ms2_mz.cpu()
            ).to(device)
